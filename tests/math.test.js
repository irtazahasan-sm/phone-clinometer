...
function runMathTests() {
    ...
    const height1 = calculateHeight(45, 10, 6);
    ...
    const height2 = calculateHeight(0, 50, 5.5);
    ...
    // Test 3: 30-degree angle
    const height3 = calculateHeight(30, 20, 6);
    const expected3 = Math.tan(30 * Math.PI / 180) * 20 + 6;
    assert(
        approximately(height3, expected3, 0.1),
        `30° angle test: expected ${expected3.toFixed(2)}ft, got ${height3.toFixed(2)}ft`
    );

    // Test 4: Realistic tree measurement scenario
    const height4 = calculateHeight(60, 30, 5.5);
    ...
    const height5 = calculateHeight(30, 40, 4);
    ...
    const height6 = calculateHeight(5, 100, 6);
    ...
    // Test 7: Default eye height
    const height7 = calculateHeight(45, 10);
    const expected7 = 10 + 5.5; // tan(45°) = 1, default eyeHeight = 5.5
    assert(
        approximately(height7, expected7, 0.1),
        `Default eye height test: expected ~${expected7}ft, got ${height7.toFixed(2)}ft`
    );
...
function runValidationTests() {
    ...
    try {
        // Test with infinity
        const result1 = calculateHeight(90, 10, 6);
        assert(
    ...
    const result2 = calculateHeight(80, 100, 6);
    assert(
        isFinite(result2) && result2 > 0,
        `Large angle test: got ${result2.toFixed(2)}ft (should be finite and positive)`
    );
